...
def _build_campaign_payload(priorities: List[str], channels: List[str]) -> List[Dict[str, Any]]:
  base_channels = channels[:4] if channels else ["WhatsApp", "Telegram", "Instagram", "LinkedIn"]
  campaigns = []
...
def _persist_campaigns_file(campaigns: List[Dict[str, Any]]):
  ...
  path.write_text(json.dumps({"campaigns": campaigns, "at": datetime.utcnow().isoformat()}, indent=2, ensure_ascii=False))


def _upsert_business_demand(session, camp: Dict[str, Any]):
  existing = session.execute(text("SELECT id FROM business_demands WHERE title=:title"), {"title": camp["title"]}).fetchone()
  if existing:
    return False
  session.execute(
    text(
...
def schedule_campaigns(session, config: Dict[str, Any]):
  ...
  created = sum(1 for camp in campaigns if _upsert_business_demand(session, camp))
