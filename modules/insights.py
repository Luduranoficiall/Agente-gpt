...
from agent_runtime import kv_get, text
...
def _fetch_scalar(
    session, query: str, params: Dict[str, Any] | None = None, default: Any = 0
) -> Any:
...
def generate_universal_dashboard(session, config: Dict[str, Any]):
    ...
    MRR = _fetch_scalar(
        session, "SELECT COALESCE(SUM(amount),0) FROM subscriptions")
    ...
    payload = {
        ...
        "prioridades_top": (
            list((config.get("prioridades_maximas") or {}).values())[:3]
