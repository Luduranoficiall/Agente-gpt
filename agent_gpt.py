...
from modules.integrations.social_channels import (ChannelName, SocialChannelError, broadcast, send_to_channel)
...
# ==== BLOCOS DE WHATSAPP PROFISSIONAL E PREMIUM ====

# ==== BLOCOS DE WHATSAPP PROFISSIONAL E PREMIUM (REDE SOCIAL FIRST) ====

def bloco_whatsapp_profissional():
    return """
======================================================
   🔵 ATENDIMENTO IMEDIATO – AGENTE SOCIAL

Fale agora mesmo com nosso time oficial pelo WhatsApp.
Suporte rápido, humano, profissional e de alta prioridade.

📞 Número oficial: +55 12 99634-1928  
🌐 Link direto: https://wa.me/5512996341928

Nossos especialistas estão preparados para ajudar você:

✔ Suporte ao usuário  
✔ Dúvidas sobre cadastro  
✔ Ajuda com integrações e automações  
✔ Informações sobre redes sociais, estratégias e negócios digitais  
✔ Atendimento estratégico para empresas, creators e influenciadores  
...
Clique e fale agora mesmo:
https://wa.me/5512996341928
======================================================
"""


def bloco_whatsapp_premium():
    return """
======================================================
        AGENTE SOCIAL • Atendimento Premium
------------------------------------------------------

Conecte-se imediatamente com nossa equipe oficial através do WhatsApp.
Nosso atendimento é humano, rápido, estratégico e focado em resultado.

📱 Canal Oficial de Suporte e Relacionamento
Número: +55 12 99634-1928
Acesso Direto: https://wa.me/5512996341928

O que você pode resolver agora mesmo:
• Ativação de integrações
• Ajuda com cadastro e automação
• Suporte ao usuário e ao cliente
• Informações sobre planos, redes sociais e negócios digitais
• Orientação para creators, empresas e influenciadores
• Direcionamento para estratégias digitais
...
Clique abaixo para ser atendido agora:
➡ https://wa.me/5512996341928
...
# Montar arquivos estáticos se existir a pasta
...
INFO_DEFAULTS = {
    "whatsapp": "+55 12 99634-1928",
    "whatsapp_link": "https://wa.me/5512996341928",
    "pix_chave": os.getenv("PIX_CHAVE", "contato@empresa.com"),
    "planos": [
        {"nome": "Profissional", "preco": 197, "descricao": "Atendimento ilimitado, suporte via WhatsApp, integração Pix e Cartão, relatórios e dashboard."},
        {"nome": "Empresarial", "preco": 497, "descricao": "Tudo do Profissional, multi-usuários, personalização avançada, suporte prioritário."}
    ]
}
...
# Middleware de segurança: CORS, HTTPS, Headers
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://agente.extraordinaria.ai", "https://www.agente.extraordinaria.ai"],
...
# Middleware de headers de segurança e forçamento de HTTPS
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        # Forçar HTTPS (redireciona para HTTPS se não estiver seguro)
        if request.url.scheme != "https":
            from starlette.responses import RedirectResponse
            # Log de tentativa insegura
            print(f"[SECURITY] Redirecionando requisição insegura para HTTPS: {request.url}")
        ...
        response.headers["Content-Security-Policy"] = (
            ...
            "connect-src 'self' https://api.agente.extraordinaria.ai; "
...
# Endpoint robusto para a raiz (landing page ou resposta amigável)
@app.get("/", response_class=HTMLResponse)
async def root():
    landing_path = Path(__file__).parent / "landing.html"
    if landing_path.exists():
        return landing_path.read_text(encoding="utf-8")
    return "<h1>Agente GPT - API Online</h1><p>Landing page não encontrada.</p>"
...
# Healthcheck robusto e checagem de variáveis críticas
@app.get("/health")
def health():
    missing_vars = []
    for var in ["DATABASE_URL", "WHATSAPP_EMPRESA", "PIX_CHAVE"]:
        if not os.getenv(var):
            missing_vars.append(var)
    return {
        "status": "warn" if missing_vars else "ok",
        "time": datetime.utcnow().isoformat(),
        "service": "Agente GPT — FULL VERSION",
        "missing_env": missing_vars,
    }

# Log de inicialização e alerta de variáveis críticas
def log_startup():
    logger.info("Iniciando Agente GPT — FULL VERSION")
    for var in ["DATABASE_URL", "WHATSAPP_EMPRESA", "PIX_CHAVE"]:
        val = os.getenv(var)
        if not val:
            logger.warning(f"Variável de ambiente ausente: {var}")
        else:
...
def whatsapp_send(inp: WhatsAppMsgIn):
    ...
    try:
        # Corrigir chamada para argumentos corretos (destination, não 'to')
        # Compatível com payloads antigos: usa inp.to se destination não existir
        destination = getattr(inp, 'destination', None) or getattr(inp, 'to', None) or ""
        result = send_to_channel(inp.channel, destination, inp.message)
...
def notify_channel(payload: SocialSingleIn):
    try:
        # Para canais que exigem destino, usar 'to' como destination se existir
        destination = getattr(payload, 'destination', None) or getattr(payload, 'to', None) or ""
        result = send_to_channel(payload.channel, destination, payload.message)
        return {"status": "ok", "channel": payload.channel, "result": result}
    except SocialChannelError as se:
        logger.error(f"[MONITORAMENTO] Falha ao notificar canal {payload.channel}: {se}")
        raise HTTPException(status_code=400, detail=str(se))
    except Exception as e:
...
def serve_css():
    ...
    # Tenta servir do static se não existir
    static_css = STATIC_PATH / "style.css"
    if static_css.exists():
        return FileResponse(str(static_css), media_type="text/css")
...
def serve_js():
    ...
    static_js = STATIC_PATH / "script.js"
    if static_js.exists():
        return FileResponse(str(static_js), media_type="application/javascript")
    return Response("JS não encontrado.", status_code=404)


# --- FORMULÁRIO DE CONTATO ---
@app.post("/enviar")
async def receber_formulario(request: Request):
    form = await request.form()
    nome = form.get("nome", "")
    email = form.get("email", "")
    mensagem = form.get("mensagem", "")
    logger.info(f"[FORM] Nome: {nome} | Email: {email} | Mensagem: {mensagem}")
    return PlainTextResponse("OK", status_code=200)

# --- EXEMPLO DE USO CORRETO PARA /notify/whatsapp ---
...
def run_cycle():
    try:
        ...
        # Checagem automática de variáveis críticas
        missing_vars = []
        for var in ["DATABASE_URL", "WHATSAPP_EMPRESA", "PIX_CHAVE"]:
            if not os.getenv(var):
                missing_vars.append(var)
        if missing_vars:
            logger.warning(f"[MONITORAMENTO] Variáveis críticas ausentes: {missing_vars}")
        else:
            logger.info("[MONITORAMENTO] Todas variáveis críticas presentes.")
        # Teste automático dos endpoints críticos (apenas existentes)
        import requests
        endpoints = [
            ("/health", "healthcheck"),
            ("/pagamento/pix", "pix"),
            ("/pagamento/cartao", "cartao_post"),
        ...
        for path, name in endpoints:
            try:
                if name == "cartao_post":
                    resp = requests.post(base_url + path, json={"numero": "4111111111111111", "validade": "12/29", "cvv": "123", "nome": "Teste"}, timeout=10)
                elif name == "whatsapp":
                    resp = requests.post(base_url + path, json={"channel": "whatsapp_cloud", "destination": "+5512996341928", "message": "Teste de WhatsApp"}, timeout=10)
                else:
                    resp = requests.get(base_url + path, timeout=10)
                if resp.status_code != 200:
                    logger.warning(f"[MONITORAMENTO] Falha endpoint {name}: status {resp.status_code} - {resp.text}")
            except Exception as e:
                logger.error(f"[MONITORAMENTO] Erro ao testar endpoint {name}: {e}")
